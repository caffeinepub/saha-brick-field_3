...
type Props = {
  ...
  vehicles: Vehicle[];
  onSave: (id: string, updates: Partial<CompleteDelivery>) => void;
  onBack: () => void;
};
...
export default function EditCompleteDelivery({
  delivery,
  vehicles,
  onSave,
  onBack,
  ...
  const [deliveryDate, setDeliveryDate] = useState<Date>(
    delivery.deliveryDate ? new Date(delivery.deliveryDate) : new Date(),
  );
  ...
  const [ratePerThousand, setRatePerThousand] = useState(
    String(delivery.ratePerThousand || 0),
  );
  const [paymentStatus, setPaymentStatus] = useState<"not-paid" | "paid">(
    delivery.paymentStatus,
  );

  // Auto-populate rate from settings when vehicleType changes
  useEffect(() => {
    ...
    if (vehicleType === "Tractor") {
      ...
      resolved =
        locType === "Outside"
          ? String(tr.outsidePerThousand || "")
          : String(tr.localPerThousand || "");
    } else {
      const wr = rates.wheelRate || {};
      resolved = String(wr.perThousand || "");
    }
    if (resolved && resolved !== "0") setRatePerThousand(resolved);
  ...
  // Bats rate from settings
  const batsRate = (() => {
    const rates = loadSbfRate();
    if (rates) {
      const safety =
        vehicleType === "Tractor"
          ? Number(rates.tractorRate?.safety100) || 0
          : Number(rates.wheelRate?.safety100) || 0;
      if (safety > 0) return safety;
    }
    return delivery.batsRate || 0;
  })();
  ...
  const batsTotal = hasBats ? (batsItem!.deliverQty / 100) * batsRate : 0;
  const bricksTotal = (totalBricks / 1000) * rate;
  const totalAmount = Math.round(batsTotal + bricksTotal);
  ...
  const handleSave = () => {
    onSave(delivery.id, {
      ...
      vehicleType,
      vehicleNumber: selectedVehicle?.number || delivery.vehicleNumber,
      loadingLabours,
      unloadingLabours,
    ...
    toast.success("Complete Delivery আপডেট হয়েছে!");
    onBack();
  };
      ...
      <div className="flex flex-col gap-3 p-4">
        ...
        <div className="bg-[#1a3c2a] rounded-2xl p-4 text-white">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            ...
            <span className="border border-green-400 text-green-300 text-[11px] font-bold px-3 py-1 rounded-full">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm">
          ...
          {/* Show current vehicle if not in list */}
          {delivery.vehicleNumber && filteredVehicles.length === 0 && (
            <div className="text-sm text-gray-600 mt-1">
              Current: {delivery.vehicleNumber}
            </div>
          )}
        </div>
